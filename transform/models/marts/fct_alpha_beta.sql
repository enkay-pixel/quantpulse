-- CAPM decomposition of the strategy against SPY, per evidence phase.
--
-- Comparing a market-neutral long/short book to SPY's raw return is apples to oranges:
-- the strategy deliberately gives up market beta, so it "should" trail the index in a
-- bull run. What actually matters is how much market exposure it carries (beta, ideally
-- ~0) and what it earns independently of the market (alpha), plus how reliably it does
-- so (information ratio). Postgres' built-in regression aggregates do the fit:
...
with joined as (
    ...
    inner join {{ ref('fct_portfolio_daily') }} as p on b.date = p.date
